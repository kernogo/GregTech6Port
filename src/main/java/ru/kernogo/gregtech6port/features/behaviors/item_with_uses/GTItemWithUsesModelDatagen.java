...
/**
 * Datagen for all Items With Uses. <br>
 * (See {@link GTItemWithUsesBehavior})
 */
public final class GTItemWithUsesModelDatagen extends ItemModelProvider {
    private final GTItemWithUsesVariantModelMaker itemWithUsesVariantModelMaker;

    public GTItemWithUsesModelDatagen(PackOutput output, ExistingFileHelper existingFileHelper) {
        super(output, GregTech6Port.MODID, existingFileHelper);
        this.itemWithUsesVariantModelMaker = new GTItemWithUsesVariantModelMaker(this);
    }
    ...
    private void make2StageLighterLikeItem(DeferredItem<Item> deferredItem) {
        itemWithUsesVariantModelMaker.make2StageVariantModel(deferredItem, "like/lighter");
    }

    private void make3StageLighterLikeItem(DeferredItem<Item> deferredItem) {
        itemWithUsesVariantModelMaker.make3StageVariantModel(deferredItem, "like/lighter");
    }

    private void make2StageSprayLikeItem(DeferredItem<Item> deferredItem) {
        itemWithUsesVariantModelMaker.make2StageVariantModel(deferredItem, "like/spray");
    }
